...
const {
  ...
  jwtVerify,
  importJWK
} = require('jose'); //Who is jose you may ask well jose is a nice JWT Key Utility tool in npm!
...
class EncryptedMemoryStore extends session.MemoryStore {
  ...
  hashIdToken(idToken) {
    const hash = crypto.createHmac('sha256', this.#salt).update(idToken).digest('hex');
    return hash;
  }
...
function configureSessionMiddleware(secret) {
  ...
  app.use(session({
    ...
    resave: true,
    // Leave false later on!
    saveUninitialized: false,
    store: sessionStore.getPrivate(),
    cookie: {
      maxAge: rotationInterval
    },
    // every 1 minutes
    rolling: false
  }));
...
const config = {
  authRequired: false,
  auth0Logout: true,
  // Enable Auth0's automatic logout
  idpLogout: true,
  secret: local1,
  baseURL: local2,
  clientID: local3,
...
const Ozv4YTHM3Uy_ = 15; // 5 minutes in milliseconds
...
app.use(async (req, res, next) => {
  if (req.oidc.isAuthenticated()) {
    ...
    if (sessionId && sessionLastActivity[sessionId]) {
      ...
      if (parseInt(currentTime / 1000 - lastActivity / 1000) > Ozv4YTHM3Uy_) {
        req.sessionTimedOut = true;
      } else {
        //Reset the time only if not in range!
        const lastActivity = Date.now();
        sessionLastActivity[sessionId] = lastActivity;
        req.sessionTimedOut = false;
      }
    } else if (sessionId && !sessionLastActivity[sessionId]) {
      //Set the time!
...
function getJWKS() {
  return new Promise((resolve, reject) => {
    fetch('https://dev-vh2t60m5403ism26.us.auth0.com/.well-known/jwks.json').then(response => {
      if (!response.ok) {
...
async function verifyLogoutToken(token) {
  ...
  const JqACg2GIbSAX = await getJWKS();
  //console.log('Verify test:', JWKS);
  try {
    const {
      payload
    } = await jwtVerify(token, async header => {
      const key = await getKey(header, JqACg2GIbSAX);
      return key;
    }, {
